...
### 5. Data Transfer Objects (DTOs) - PADRÃO SOLID

**Quando usar DTOs:**
- Quando a API retorna muitos atributos (> 10) mas a View usa apenas alguns (< 7)
- Para seguir o **Princípio de Segregação de Interface (ISP)** do SOLID
- Para encapsular dados e expor apenas o necessário à UI

**Estrutura Correta (SOLID):**
- **Model** (`[feature]_model.dart`): Entidade completa com TODOS os atributos da API
- **DTO** (`[feature]_dto.dart`): Versão simplificada apenas com campos essenciais para View/UI
- **Repository**: Retorna Model completo; DTO é criado na camada de apresentação

**Responsabilidades:**
- **Model**: Representa a entidade de domínio completa, deserializa API, persiste dados
- **DTO**: Objeto de apresentação leve, contém apenas dados para renderização
- **Repository**: Gerencia acesso a dados e retorna Models

**Exemplo Completo (Estratégia Correta):**

```dart
// ✅ main_content_topic_model.dart (Entidade completa - 23 campos)
class MainContentTopicModel implements BaseModel {
  // TODOS os 23 campos da API
  final String id;
  final String title;
  final String description;
  ...
  factory MainContentTopicModel.fromMap(Map<String, dynamic> map) {
    return MainContentTopicModel(
      ...
      channelId: map['channelId'] as String?, // ✅ Nullable fields
      // ... mapear TODOS os campos
    );
  }

  Map<String, dynamic> toMap() { /* serializar todos */ }
  MainContentTopicModel copyWith({...}) { /* clonar */ }
}
```

```dart
// ✅ main_content_topic_dto.dart (Apresentação - 7 campos)
/// DTO simplificado para a camada de Apresentação (View/UI)
/// Contém APENAS os campos essenciais usados pela View
/// Segue o princípio ISP (Interface Segregation Principle) do SOLID
...
class MainContentTopicDto {
  ...
  /// SRP (Single Responsibility): Conversão Model -> DTO
...
```dart
// ✅ main_content_topic_repository.dart
class MainContentTopicRepository {
  Future<List<MainContentTopicModel>> getAll() async {
    ...
    if (response.statusCode == 200) {
      ...
      // Retorna Models completos (não DTO aqui)
      return data.map((json) {
        return MainContentTopicModel.fromMap(json);
      }).toList();
    }
...
```dart
// ✅ main_content_topic_view_model.dart (Usa DTO quando necessário)
class MainContentTopicViewModel extends ChangeNotifier {
  List<MainContentTopicModel> _models = [];
  
  // Expor Models completos para operações de negócio
  List<MainContentTopicModel> get models => _models;
  
  // OU: Expor DTOs simplificados apenas para a View
  List<MainContentTopicDto> get dtos => 
    _models.map((m) => MainContentTopicDto.fromModel(m)).toList();
}
```

**Vantagens do Padrão SOLID:**
1. **SRP**: Model gerencia dados completos, DTO gerencia apresentação
2. **ISP**: View não depende de campos desnecessários
3. **Encapsulamento**: Lógica de mapeamento isolada no DTO
4. **Performance**: View trabalha com objetos leves (7 campos vs 23)
5. **Flexibilidade**: Múltiplos DTOs (ListDTO, DetailDTO) para o mesmo Model

**Regras:**
- ✅ **Model** tem TODOS os campos da API + `fromMap()` + `toMap()` + `copyWith()`
- ✅ **DTO** tem campos essenciais + `fromModel()` (factory)
- ✅ **Nomes dos atributos** no DTO devem ser IDÊNTICOS aos da API REST (sem mapeamento semântico)
- ✅ **Repository** retorna Model, não DTO
- ✅ **ViewModel** pode expor DTO para View quando necessário
- ❌ **NUNCA** deserializar API diretamente para DTO
- ❌ **NUNCA** usar DTO no Repository (apenas Model)
- ❌ **NUNCA** usar nomes customizados no DTO (ex: `subtitle` em vez de `channelName`)
